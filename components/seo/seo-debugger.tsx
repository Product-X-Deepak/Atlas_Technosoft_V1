...
function SEODebugger() {
  ...
  return (
    <Card className="border border-accent-300 shadow-md mb-6 mt-20">
      <CardHeader className="bg-accent-50 border-b border-accent-200">
        <div className="flex items-start justify-between">
          <div>
            <CardTitle className="text-accent-800">SEO Debugger</CardTitle>
            <CardDescription className="text-accent-700">
              Inspect structured data, metadata, and SEO elements
            </CardDescription>
          </div>
          <Badge variant="outline" className="bg-white text-accent-800 border-accent-300">
            Development Only
            </Badge>
        </div>
        </CardHeader>
          ...
          <TabsContent value="schemas" className="space-y-4">
            {validatingSchema && (
              <div className="flex items-center space-x-2 text-sm text-slate-500">
                <div className="h-3 w-3 rounded-full bg-accent-500 animate-pulse"></div>
                <span>Validating structured data...</span>
              </div>
            )}
            
            {validationError && (
              <div className="p-3 bg-accent border border-accent rounded-md text-primary text-sm">
                ...
                >
                  {validationResults.results.map((result, index) => (
                    <AccordionItem 
                      key={`schema-${index}`}
                      value={`schema-${index}`}
                      className={`border ${!result.valid ? 'border-accent bg-accent' : 'border-slate-200'} rounded-md overflow-hidden`}
                    >
                      <AccordionTrigger 
